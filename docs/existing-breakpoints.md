...
리뉴얼 HTML(`css/intro.css`)은 위 기존 경계를 그대로 쓴다.

| | 기존 라이브 | 작업본 |
|---|---|---|
| 태블릿 레이아웃 | `max-width: 1024px` / `601–1024px` | 동일 |
| 모바일 레이아웃 | `max-width: 600px` | 동일 |
| 유틸 클래스 | `gt-sm`, `lt-md`, `lt-sm`, `gt-xs` | 동일 정의·동일 용도 |

헤더 버튼/햄버거, 앱 QR, 푸터 SNS는 유틸 클래스로 켜고 끈다. 그리드·타이포는 레이아웃 미디어쿼리로 접는다.
